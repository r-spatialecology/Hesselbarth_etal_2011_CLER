...
Because additionally to handling spatial data, also further tasks such as statistical modeling, preparation of publication-ready figures and even preparation of complete reports can be done in the same working environment, the *R* programming language can be a very powerful tool.
Thus, in the first part we present an overview over existing *R* packages for landscape ecology.
...
```{r fig-workflow, echo = FALSE, out.width = "100%", out.height = "65%", fig.align="center", fig.cap = "Exemplary worklow of spatial data analyses for landscape ecology using the R programming language. For all major tasks, a few example *R* packages are listed. Of course, many more packages for the specific tasks exist."}
include_graphics("data/Workflow.pdf")
```
...
<!-- MH: I would argue to only include packages that are actively maintained. It's already such a huge amount and I guess there is a reason why the packages were archived. Doesnt speak for their popularity -->

## Spatial data representations {#sec:spatrep}

<!-- MH: Make sure all libraries etc. have reference -->
...
<!-- MH: I probably would not add terra here. As far as I see, the vector support in terra is basically just support to combine vector with raster data. The main focuse of the package is raster data though -->
...
Since public available data is becoming more and more prominent, so are *R* packages to accesses them.
Packages include the *FedData* package \cite{R-FedData} to access the National Land Cover Database (NLCD) data for the USA, the *elevatr* package to access raster elevation data based on the Amazon Web Services Terrain Tiles \cite{R-elevatr}, the *rgbif* to access the Global Biodiversity Information Facility (GBIF) portal \cite{R-rgbif}, *rnaturalearth* to access the Natural Earth database \cite{South2017}, or *marmap* to download bathymetry data from the ETOPO1 database \cite{Pante2013}.
Furthermore, the `get_data()` function from the *raster* package allows to download climatic and bioclimatic data from WorldClim v1.4, while the new in-development package *geodata* <!--(add reference)--> provides an access to data from WorldClim v2.
...
<!-- MH: Again, I dont think thats really of relevance  -->
...
<!-- JN: I am not sure if that's true as few NLMR functions are just wrappers around existing R functions from RandomFields. Also, it is possible to simulate landscapes (gradient landscapes) using geostatistical packages, e.g., gstat. --> <!-- MH: Well, but RandomFields and geostatistcal are not directly designed for NLM. Maybe need to relax the statement a little bit --> Using a wide collection of algorithms, the *NLMR* package \cite{Sciaini2018} provides framework to simulate neutral landscape rasters.

\begin{table}

\caption{Overview of commonly used \textit{R} packages for landscape ecology. Packages are sorted by their major application task. Only packages focused on spatial data and landscape ecology are included.} 
