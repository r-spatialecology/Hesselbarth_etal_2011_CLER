...
- name: Johannes M. Signer
  address: Department of Wildlife Sciences, University of Göttingen, Büsgenweg 3, 37077 Göttingen, Germany
  email: j.m.signer@gmail.com
- name: Laura J. Graham
  address: Geography and Environment, University of Southampton, Highfield Rd, Southampton SO17 1BJ, United Kingdom
...
<!-- JN: my first thought was - "do we even need this kind of introduction in the 'Current Landscape Ecology Reports' journal"; however, after a while, I like it a lot. It could be very helpful for people from other fields. -->
<!-- MH: Yeah, I kind of like it. It sets the stage I think. Also, I hope the paper will be read by people that are not really familiar with landscape ecology as a field and just came across some spatial data -->
...
While human activities have altered the landscapes for millenniums \cite{Ellis2011,Ellis2015}, in the past centuries, the effects of humans on landscapes have increased to an unknown high, known as the Anthropocene \cite{Crutzen2002}.
...
Last but not least, even though not strictly necessary by definition \cite{Steiniger2009,Steiniger2009a}, most open-source software is also free-of-cost, which allows using the software without any costs in contrast to often expensive proprietary software \cite{vonKrogh2006,Steiniger2009,Steiniger2009a}.
This democratize scientific research as free-of-cost software removes one gatekeeper for researches without access to proprietary software.
...
<!--JN: language is older (1991?), but in 1995 it was officially released for the first time-->
<!-- MH: Since we write "released" I think it's ok to use 1995? Especially since this was most likely the time a broader audience used it? -->
...
The growing popularity of the *R* programming language for spatial data analysis and landscape ecology can also be seen with the increasing number of related textbooks \cite{Fletcher2019,Lovelace2019,Pebesma2019a}.
<!--JN: should we cite Pebesma2019a as it is a work in progress? -->
<!-- MH: I think this does not speak against citing it, but also happy to remove it -->
<!--JN: also - maybe we should cite http://book.ecosens.org/RSEbook/?-->
<!-- MH: I dont know it, but sure if it fits -->
...
Even though many other open-source tools for landscape ecology exist \cite{QGISDevelopmentTeam2016,GRASSDevelopmentTeam2017,Porta2017}, in this review, we focus on software implemented in the *R* programming language.
...
Because in addition to handling spatial data, also tasks such as statistical modeling, creation of publication-ready figures, and even preparation of complete reports can be done in the same working environment (Fig. \@ref(fig:fig-workflow)), the *R* programming language can be a very powerful tool.
...
While *R* has several build-in data structures, including vectors, matrices, data.frames, and lists, it has no internal support for reading, processing, or visualizing spatial data.
...
Importantly, main *R* packages for spatial data use the external *GDAL* and *PROJ* libraries, which allow for reading and writing of hundreds of spatial data formats, and coordinates transformation.
<!--JN: references??-->
...
Packages include *rnaturalearth* \cite{South2017} to access the Natural Earth database, the *elevatr* package to access raster elevation data \cite{R-elevatr}, the *rgbif* to access the Global Biodiversity Information Facility (GBIF) portal \cite{R-rgbif}, the *BIEN* package \cite{Maitner2020} to access the Botanical Information and Ecology Network Database, the *marmap* to download bathymetry data from the ETOPO1 database \cite{Pante2013}, or the *FedData* package \cite{R-FedData} to access the National Land Cover Database (NLCD) data for the USA.
Furthermore, the *getlandsat* package \cite{Chamberlain2018} allows to download data from the Landsat 8, the *MODIS* package \cite{Mattiuzzi2020} to download MODIS products, and *sen2r* \cite{R-sen2r} to download  Sentinel-2 optical images.
Also, the `getData()` function from the *raster* package allows to download climatic and bioclimatic data from WorldClim v1.4 (for WorldClim v2, see the in-development package *geodata* <!--(add reference) https://github.com/rspatial/geodata-->).
...
All packages from Section \@ref(sec:spatrep) have interfaces for coordinates transformations, allowing to unify spatial projections when the used data have different CRS.
...
Related to that, masking certain areas of the study region can be required by using, e.g., certain spatial filters such as water bodies.
...
The package allows calculating the most widely used landscape metrics in a transparent and reproducible workflow within the *R* environment.
...
The *geodiv* package \cite{Smith2020} allows to calculate gradient surface metrics to characterize continuous analysis of landscape features. 
Additionally, the *belg* package allows calculating the Boltzmann entropy of a landscape gradient \cite{R-belgpaper}.

Most landscape metrics are a single number depicting specific characteristics of a local landscape.
...
Species distribution modeling (SDM) examine how landscape patterns (e.g., habitat suitability or resources availability) influence and determine the patterns of species distribution, mainly to infer ecological processes and predict future species distributions \cite{Wiersma2011}.
...
More general, the *igraph* package \cite{Csardi2006} provides functionality related to graph theory.
Resistance surfaces and least-cost paths are other tools to model functional connectivity.
...
Absorbing Markov chains that combine random walk theory and mortality based on the landscape characteristics \cite{Fletcher2019a} and is provided by the recently published *samc* package \cite{Marx2020}.
...
By explicitly including landscape characteristics, landscape genetics is a way more realistic way of analysis than, e.g., metapopulation genetics \cite{Holderegger2006}.
