...
R packages & Software extension to R programming language \\
SDM & Species distribution modelling \\
vector & Points and lines based data model \\
...
The expanding CRAN Task Views document this: *Analysis of Spatial Data* \cite{Bivand2019a} and *Handling and Analyzing Spatio-Temporal Data* \cite{Pebesma2020} currently list about 300 packages in total.
...
Packages include *rnaturalearth* \cite{South2017} to access the Natural Earth database, the *elevatr* package to access raster elevation data \cite{R-elevatr}, the *rgbif* package to access the Global Biodiversity Information Facility (GBIF) portal \cite{R-rgbif}, the *BIEN* package \cite{Maitner2020} to access the Botanical Information and Ecology Network Database, the *marmap* to download bathymetry data from the ETOPO1 database \cite{Pante2013}, or the *FedData* package \cite{R-FedData} to access the National Land Cover Database (NLCD) data for the USA.
...
Geostatistics, or spatial statistics, are complimentary to landscape metrics, and can be used to analyze patterns in continuous data (e.g. normalized difference vegetation index, disturbance intensity, topography). 
...
The *spatstat* package contains functionality for point pattern analysis, including exploratory analysis; simulation of point process models; and modelling fitting, inference and diagnostics. 
A comprehensive text book covering both theoretical background as well as applied examples can be found here \cite{Baddeley2015}.
...
The *spdep* package has methods for quantifying multiple metrics of spatial autocorrelation, and to correct for these in a spatial autoregressive model.
The *rinla* and *inlabru* packages also provide functionality for modelling of spatially structured data. 

Finally, with variograms and correlograms, the spatial structure of continuous landscapes can be quantified and compared. 
R packages *geoR* and *gstat* provide functionality for this type of analysis, as well as interpolation methods, known as (co-)kriging. 
...
\begin{tabularx}{450pt}{lrrr}

\hline
Task & R package & Describtion & Reference \\
\hline
\multirow{8}{*}{Spatial data} & raster & Raster data handling and analysis & \cite{Hijmans2019} \\
& terra & Basic data handling and analysis & \cite{R-terra} \\
& stars & Spatiotemporal raster data & \cite{Pebesma2019} \\
...
& FedData & Download US federal data & \cite{R-FedData} \\
...
& belg & Calculates the Boltzmann entropy & \cite{R-belgpaper} \\
& motif & Not on CRAN? & \cite{R-motif} \\
& geodiv & Quantify continuous landscape patterns & \cite{Smith2020} \\
\hline
\multirow{8}{*}{\shortstack[l]{Species distribution\\modeling}} & dismo & Add describtion & \cite{Hijmans2017} \\
& sdm & Add describtion & \cite{Naimi2016} \\
& ecospat & Add describtion & \cite{Broennimann2020} \\
& biomod2 & Add describtion & \cite{Thuiller2020} \\
& PresenceAbsence & Add describtion & \cite{Freeman2008} \\
& zoon & Add describtion & \cite{Golding2018} \\
& adehabitatHR, adehabitatHS & Add describtion & \cite{Calenge2006} \\
& amt &  Add describtion &\cite{Signer2019} \\
\hline
\multirow{5}{*}{Connectivity} & lconnect & Add describtion & \cite{Mestre2019} \\
& Makurhini & Add describtion & \cite{Godinez-Gomez2020} \\
& grainscape & Add describtion & \cite{Chubaty2020} \\
& gdistance & Add describtion & \cite{vanEtten2017} \\
& samc & Add describtion & \cite{Marx2020} \\
\hline
\multirow{4}{*}{Landscape genetics} & raphs4lg & Add describtion & \cite{Savary2020} \\
& PopGenReport & Add describtion & \cite{Adamack2014,Gruber2015} \\
& HierDpart & Add describtion & \cite{Qin2019} \\
& GeNetIt & Add describtion & \cite{Murphy2010} \\
\hline
\multirow{4}{*}{various} & NLMR & Simulate neutral landscape models & \cite{Sciaini2018} \\
& RandomFields & Add describtion & \cite{Schlather2015} \\
