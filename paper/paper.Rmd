...
```{r data-model-packages, include = FALSE}
# data model
...
ggplot_packages <- ggplot(data = r_packages_df) +
  ...
  labs(x = "Number of package mentionings [%]", y = "List the three R packages related to landscape ecology you use the most!")
```
...
This was also represented in the most used *R* packages (Fig. \@ref(fig:fig-packages-tasks) A))
When asked for the three most used packages, participants of the survey listed `r length(unique(r_packages))` packages in total. The *raster* packages was listed the most, followed by the *sf* package. 
Because both the *raster* and the *sf* are designed for basic and advanced data handling and processing of raster and vector data, respectively, it is not surprising that "(pre-)processing of spatial data" was the most important workflow task for the majority of the survey participants (Fig. \@ref(fig:fig-packages-tasks) B)). 
...
ggplot_tasks <- ggplot(data = tasks_df) +
  ...
  scale_colour_manual(values = c("grey50", "black")) +
  guides(fill = FALSE, colour = FALSE) +
  labs(x = "Number of task mentionings [%]", y = "Which are the most important tasks to your workflow?")
```

```{r fig-packages-tasks, echo = FALSE, fig.align="center", fig.cap = "A) Most used R packages and B) most important workflow tasks as listed by the participants of the survery. 'others' includes all packages/tasks with less than five total mentions."}

ggplot_packages + ggplot_tasks + plot_layout(ncol = 2, nrow = 1) + plot_annotation(tag_levels = "A", tag_suffix = ")")
...
When asked "`r old_names[12]`", the vast majority of participants answered with either "very useful" or "useful" (`r good_eval`%) and only only very few participants evaluated *R* as "intermediate", "not useful" or "not useful at all" (`r bad_eval`%; Fig. \@ref(fig:fig-usefulness)).

```{r fig-usefulness, echo = FALSE, fig.align="center", fig.cap = "Evaluation of the usefullnes o fR for landscape ecology by the particpants of the survey. Answers were possible on a 5-point scale."}
ggplot(data = usefulness_df) +
  geom_bar(aes(x = usefulness_class, y = n_rel), 
           stat = "identity") + 
  theme_classic(base_size = 7.5) + 
  scale_y_continuous(breaks = seq(0, 60, 5), limits = c(0, 60)) + 
  guides(fill = FALSE, colour = FALSE) +
  labs(x = "How useful do you think is R currently for landscape ecology?", 
