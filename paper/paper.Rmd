...
<!-- JN:  We still can slightly update the landscape ecology introduction, but I think that the lenght of this text is good. Three paragraphs seems enough, as our goal is not to explain what is the landscape ecology, but what are the tools used in this field. -->
...
<!--probably we should also mention the cost here-->
...
Lastly, open-source software allows other scientists to reuse code and not "reinvent the wheel" \cite{Prlic2012} and to customize existing software to their specific needs \cite{Steiniger2009}. 

One example of a successful open-source project is the *R* programming language and its *Comprehensive R Archive Network* (CRAN) for extensions (also called packages) \cite{RCoreTeam2019}. 
...
<!-- we should add that the total number of spatial-related packages is, in fact, larger, as the task views, are manual lists, updated just by a few people -->
...
Even though many other open-source tools for landscape ecology exist \cite{QGISDevelopmentTeam2016,GRASSDevelopmentTeam2017,Porta2017}, in this review we are going to focus on tools implemented in the *R* programming.
<!-- probably we should state why we are focusing on R only -->
<!-- some answers: "vastness of spatial R packages", ... -->
...
<!-- JN: maybe we can also add info about RMarkdown and similar, with R as an integrative tool for making scientific outputs -->
...
<!-- JN: maybe we should also add an introduction to R packages here-->
<!-- for example, CRAN vs GitHub vs other -->
<!-- who creates R packages? -->
<!-- who maintains them and what is an archived package -->
<!-- resources on how to create your own packages - https://r-pkgs.org/ -->
<!-- comment on never-ending changes in r world -->

## Spatial data representations {#spatrep}
...
All of the packages listed in Section \@ref(spatrep) have build-in methods for plotting spatial data using the `plot()` function.
...
Popular approaches and packages include generalized linear models using, e.g., the *stats* package; generalized additive models using, e.g., the *mgcv* or *lme4* package; classification and regression trees (CART) using, e.g., the *rpart*, *randomForest* or *ranger* package or multivariate data analysis using , e.g., the *ade4* or *vegan* package. 
Of course, also packages specifically designed for SDM exists. 
This includes the *dismo*, *sdm*, *ecospat*, *biomod2* and *PresenceAbsence* packages.
<!-- JN: I think all of the above packages should be cited -->
...
<!--JN: I am not sure if that's true as few NLMR functions are just wrappers around existing R functions from RandomFields.
Also, it is possible to simulate landscapes (gradient landscapes) using geostatistical packages, e.g., gstat.
-->
