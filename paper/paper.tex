...
Because additionally to handling spatial data, also further tasks such as statistical modeling, preparation of publication-ready figures and even preparation of complete reports can be done in the same working environment, the \emph{R} programming language can be a very powerful tool.
Thus, in the first part we present an overview over existing \emph{R} packages for landscape ecology.
...
{\centering \includegraphics[width=1\linewidth,height=0.65\textheight]{data/Workflow} 

}

\caption{Exemplary worklow of spatial data analyses for landscape ecology using the R programming language. For all major tasks, a few example *R* packages are listed. Of course, many more packages for the specific tasks exist.}\label{fig:fig-workflow}
\end{figure}
...
Most people use \emph{R} either ``daily'' (54.4\%) or a ``few times a week'' (36.9\%) and about half of all participants described themselves as ``advanced'' user (46.6\%), while 40.8\% described themselves as ``intermediate'' users.
...
Interestingly, most people use the raster data model (72.8\%) in comparison to the vector data model (27.2\%).
...
Overall, 20.4\% of the participants reported that currently no packages and functionality are missing for them or they lack the overview to answer the question.
There were two most common topics across the answers of the participants.
Firstly, many participants (12.6\%) wished for a better computational performance of \emph{R} in terms of speed and required RAM for larger data.
Secondly, many participants (8.7\%) are currently missing advanced and easy-to-apply methods for creating high-quality maps or other visualization-related functionality.
Fewer participants were missing tools for various connectivity-related issues (8.7\%) and characterization of landscape characteristics (6.8\%).
